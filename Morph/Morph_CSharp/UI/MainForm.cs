...
namespace ParallelMorph
{
    ...
    public partial class MainForm : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Get the list of available accelerators and populate the menu
            foreach(MorphLibrary.Accelerator acc in MorphLibrary.ListAccelerators())
            {
                ToolStripMenuItem item = new ToolStripMenuItem();
                item.Tag = acc;
                item.Text = "C++ AMP on " + acc.Description;
                item.Click += new System.EventHandler(this.parallelModeToolStripMenuItem_Click);
                processingModeToolStripMenuItem.DropDownItems.Add(item);
            }
        }
        ...
        private void btnMorph_Click(object sender, EventArgs e)
        {
            ...
            Debug.Assert(_morpher == null);
            if (cppSequentialProcessingModeToolStripMenuItem.Checked)
            {
                _morpher = new ComputeMorphCpp(settings, MorphLibrary.Mode.Sequential, null, cancellationToken);
                this.Text += cppSequentialProcessingModeToolStripMenuItem.Text;
            }
            else if (cppPplProcessingModeToolStripMenuItem.Checked)
            {
                _morpher = new ComputeMorphCpp(settings, MorphLibrary.Mode.PPL, null, cancellationToken);
                this.Text += cppPplProcessingModeToolStripMenuItem.Text;
            }
            else
            {
                foreach (ToolStripMenuItem item in processingModeToolStripMenuItem.DropDownItems)
                {
                    if(item.Checked)
                    {
                        _morpher = new ComputeMorphCpp(settings, MorphLibrary.Mode.CppAMP, (MorphLibrary.Accelerator)item.Tag, cancellationToken);
                        this.Text += item.Text;
                        break;
                    }
                }
